...
- `a1b58fe` — focused test in `src/realtime/server_session_events.rs`.
- `358b33d` — integration harness in
  `tests/realtime_server_session_events_contract.rs`.
- `01caabb` — event values and implementation in
  `src/realtime/server_session_events.rs`.
- This report is a separate report-only commit.
